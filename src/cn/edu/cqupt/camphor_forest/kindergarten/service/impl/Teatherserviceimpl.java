...
public class Teatherserviceimpl implements Teatherservice{
	...
	public List<Teacher> selteacher(String ss) throws SQLException {
		...
		}else if(!tdi.selectByName(ss).isEmpty()) {
        	 return (List<Teacher>) tdi.selectByName(ss);
         }else if(!tdi.selectByClassId(ss).isEmpty()){
        	 return  (List<Teacher>) tdi.selectByClassId(ss);
         }else{
        	 return null;
         } 
	}
